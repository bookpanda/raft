...
// CommitEntry: entry sent to the commit channel
// each entry = consensus was reached on a command, can be applied to client's state machine
...
type ConsensusModule struct {
	...
	// newCommitReadyChanWg is used to wait for all goroutines that send on newCommitReadyChan, for clean shutdown
	newCommitReadyChanWg sync.WaitGroup

	// trigger sending AEs to followers, no need to wait for heartbeat intervals
	...
	commitIndex        int // if an index is replicated by a majority, commitIndex advances to it.
	lastApplied        int
	state              CMState
	electionResetEvent time.Time

	// Volatile Raft state on leaders
	// next log entry index that the leader will send to each peer
	nextIndex map[int]int

	// highest log entry known to be replicated on each peer
	// used to check if a majority has replicated an entry, if yes -> commit
...
// only leader can receive Submit requests, return index where the command was added to the log
...
func (cm *ConsensusModule) Stop() {
	...
	cm.newCommitReadyChanWg.Wait() // wait for commitChanSender to finish
}
...
func (cm *ConsensusModule) RequestVote(args RequestVoteArgs, reply *RequestVoteReply) error {
	...
	if args.Term > cm.currentTerm { // receiver is behind others
	...
	if cm.currentTerm == args.Term &&
		...
		(args.LastLogTerm > lastLogTerm || // candidate's log is up-to-date
			(args.LastLogTerm == lastLogTerm && args.LastLogIndex >= lastLogIndex)) {
		reply.VoteGranted = true
		cm.votedFor = args.CandidateId
		cm.electionResetEvent = time.Now()
	} else { // never vote for older term or already voted for someone else
...
func (cm *ConsensusModule) AppendEntries(args AppendEntriesArgs, reply *AppendEntriesReply) error {
	...
	if args.Term == cm.currentTerm {
		...
		// or is PrevLogIndex -1, meaning no previous log entry
		if args.PrevLogIndex == -1 ||
			...
			if newEntriesIndex < len(args.Entries) {
				...
				// if follower log has conflicting entries, they are deleted
			...
			// mo match for PrevLogIndex/PrevLogTerm
			// populate ConflictIndex/ConflictTerm to help the leader bring us up to date quickly
...
func (cm *ConsensusModule) runElectionTimer() {
	...
	// loops until expires -> become candidate
	...
	for {
		...
		// expires: starts election if no RPCs from leader or hasn't voted for anyone
...
func (cm *ConsensusModule) startElection() {
	...
	// send RequestVote to others concurrently
...
func (cm *ConsensusModule) startLeader() {
	...
	// sends AEs every 50ms or when triggerAEChan is triggered
				...
				if cm.state == Leader && savedCurrentTerm == reply.Term {
					if reply.Success {
						...
						for i := cm.commitIndex + 1; i < len(cm.log); i++ {
							if cm.log[i].Term == cm.currentTerm {
								...
								if matchCount*2 > len(cm.peerIds)+1 {
									cm.commitIndex = i // commitIndex advances to the highest index that is replicated by a majority
								}
							}
						}
						...
						if cm.commitIndex != savedCommitIndex {
							...
							cm.mu.Unlock()
							cm.newCommitReadyChan <- struct{}{} // notify that new commits are ready
							cm.triggerAEChan <- struct{}{}
						} else {
							cm.mu.Unlock()
						}
...
// expects cm.mu to be locked before calling
...
// runs in background routine
// watches newCommitReadyChan for new commits
// sends committed entries to the commit channel
